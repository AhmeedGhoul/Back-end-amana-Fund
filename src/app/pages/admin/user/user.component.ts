...
export class UserComponent implements OnInit {
  ...
  promoteUser(user: User, role: string): void {
    // Token is now handled by the AuthInterceptor
    this.userService.promoteUser(user.email, role).subscribe({
      next: () => this.loadUsers(),
  ...
  demoteUser(user: User, role: string): void {
    // Token is now handled by the AuthInterceptor
    this.userService.demoteUser(user.email, role).subscribe({
      next: () => this.loadUsers(),
